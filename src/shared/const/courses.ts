// import img1 from '../../shared/assets/img/media/1.jpeg';
// import img2 from '../../shared/assets/img/media/2.jpeg';
// import img3 from '../../shared/assets/img/media/3.jpeg';
// import img4 from '../../shared/assets/img/media/4.jpeg';
// import img5 from '../../shared/assets/img/media/5.jpeg';
// import img6 from '../../shared/assets/img/media/6.jpeg';
// import img7 from '../../shared/assets/img/media/7.jpeg';
// import img8 from '../../shared/assets/img/media/8.jpeg';
// import img9 from '../../shared/assets/img/media/9.jpeg';
// import img10 from '../../shared/assets/img/media/10.jpeg';
// import img11 from '../../shared/assets/img/media/11.jpeg';
// import img12 from '../../shared/assets/img/media/12.jpeg';
// import img13 from '../../shared/assets/img/media/13.jpeg';
// import img14 from '../../shared/assets/img/media/14.jpeg';
// import img15 from '../../shared/assets/img/media/15.jpeg';
// import img16 from '../../shared/assets/img/media/16.jpeg';
// import img17 from '../../shared/assets/img/media/17.jpeg';
// import img18 from '../../shared/assets/img/media/18.jpeg';
// import img19 from '../../shared/assets/img/media/19.jpeg';
// import img20 from '../../shared/assets/img/media/20.jpeg';
// import img21 from '../../shared/assets/img/media/21.jpeg';
import img22 from '../../shared/assets/img/media/22.jpeg';
import img23 from '../../shared/assets/img/media/23.jpeg';
// import img24 from '../../shared/assets/img/media/24.jpeg';
import img25 from '../../shared/assets/img/media/25.jpeg';
// import img26 from '../../shared/assets/img/media/26.jpeg';
// import img27 from '../../shared/assets/img/media/27.jpeg';
import img28 from '../../shared/assets/img/media/28.jpeg';
import img29 from '../../shared/assets/img/media/29.jpeg';
import img30 from '../../shared/assets/img/media/30.jpeg';
// import img31 from '../../shared/assets/img/media/31.jpeg';
// import img32 from '../../shared/assets/img/media/32.jpeg';
// import img33 from '../../shared/assets/img/media/33.jpeg';
import img34 from '../../shared/assets/img/media/34.jpeg';
import img35 from '../../shared/assets/img/media/35.jpeg';
import img36 from '../../shared/assets/img/media/36.jpeg';
import img37 from '../../shared/assets/img/media/37.jpeg';
// import img38 from '../../shared/assets/img/media/38.jpeg';
// import img39 from '../../shared/assets/img/media/39.jpeg';
// import img40 from '../../shared/assets/img/media/40.jpeg';
// import img41 from '../../shared/assets/img/media/41.jpeg';
// import img42 from '../../shared/assets/img/media/42.jpeg';
// import img43 from '../../shared/assets/img/media/43.jpeg';
import img44 from '../../shared/assets/img/media/44.jpeg';
import kids3 from '../../shared/assets/img/media/kids3.jpeg';
import kids4 from '../../shared/assets/img/media/kids4.jpeg';
import kids5 from '../../shared/assets/img/media/kids5.jpeg';
import kids6 from '../../shared/assets/img/media/kids6.jpeg';
import Individual_teens1 from '../../shared/assets/img/media/Individual_teens1.jpeg';
import Individual_teens2 from '../../shared/assets/img/media/Individual_teens2.jpeg';
import Individual_adults1 from '../../shared/assets/img/media/Individual_adults1.jpeg';
import Individual_adults2 from '../../shared/assets/img/media/Individual_adults2.jpeg';
import Individual_adults3 from '../../shared/assets/img/media/Individual_adults3.jpeg';
import Individual_adults4 from '../../shared/assets/img/media/Individual_adults4.mp4';
import Business_English1 from '../../shared/assets/img/media/Business_English1.jpeg';
import Business_English2 from '../../shared/assets/img/media/Business_English2.jpeg';
import Business_English3 from '../../shared/assets/img/media/Business_English3.jpeg';
import English_programmers2 from '../../shared/assets/img/media/English_programmers2.jpeg';
import English_programmers1 from '../../shared/assets/img/media/English_programmers1.jpeg';
import English_programmers3 from '../../shared/assets/img/media/English_programmers3.mp4';
import Group_adults1 from '../../shared/assets/img/media/Group_adults1.mp4';
import Medical_English1 from '../../shared/assets/img/media/Medical_English1.jpeg';
import Medical_English2 from '../../shared/assets/img/media/Medical_English2.mp4';
import Group_kids1 from '../../shared/assets/img/media/Group_kids1.mp4';
import Group_kids2 from '../../shared/assets/img/media/Group_kids2.mp4';
import Individual_kids1 from '../../shared/assets/img/media/Individual_kids1.mp4';
import Business_English4 from '../../shared/assets/img/media/Business_English4.mp4';
import Speaking_club1 from '../../shared/assets/img/media/Speaking_club1.mp4';
import ege_4 from '../../shared/assets/img/media/ege_4.jpeg';
import ege_3 from '../../shared/assets/img/media/ege_3.jpeg';
import ege_2 from '../../shared/assets/img/media/ege_2.jpeg';
import ege_1 from '../../shared/assets/img/media/ege_1.jpeg';
import oge_1 from '../../shared/assets/img/media/oge_1.jpeg';
import oge_2 from '../../shared/assets/img/media/oge_2.jpeg';
import oge_3 from '../../shared/assets/img/media/oge_3.jpeg';
import oge_4 from '../../shared/assets/img/media/oge_4.jpeg';
import oge_5 from '../../shared/assets/img/media/oge_5.jpeg';

export const courses = [
    {
        id: 1,
        title: 'Групповое обучение для детей и подростков',
        imagePaths: [
            img28,
            kids3,
            Group_kids1,
            Group_kids2,
            kids4,
            kids5,
            kids6,
        ],
        description: `Идеальный старт изучения английского языка для детей и подростков — в комфортной, дружеской атмосфере мини-группы. Наши программы разработаны с учётом возраста, уровня знаний и интересов детей.
                    Современные групповые курсы английского для детей и подростков (до 5 человек в группе):
                    Интенсивность на выбор: 2 или 3 раза в неделю.
                    При занятиях 3 раза в неделю — два урока с носителем языка, один — с российским филологом.
                    При занятиях 2 раза — один урок с носителем, один с российским преподавателем.
                    Маленькие группы позволяют уделить внимание каждому ребёнку.
                    Носители языка помогут улучшить произношение и разговорную речь, а российский педагог — объяснит грамматику и тонкости языка доступно и понятно.
                    Дайте своему ребёнку возможность уверенно говорить по-английски — начните обучение в удобное время!`,
    },
    {
        id: 2,
        title: 'Групповое обучение. Взрослое отделение',
        imagePaths: [img23, img34, Group_adults1, img44],
        description: `Освойте английский язык в комфортной компании единомышленников. Групповые занятия — это эффективная практика общения, мотивация и быстрый рост навыков в удобном графике.
            Групповые занятия для взрослых (до 4 человек):
            Занятия 2 или 3 раза в неделю по вашему выбору.
            Комбинация носителей языка и российских филологов.
            Форматы:
            3 раза в неделю: два занятия с носителем, одно — с российским преподавателем.
            2 раза в неделю: одно занятие с носителем, одно — с российским филологом.
            Работаем над разговорными навыками, грамматикой, лексикой и уверенностью в общении на английском языке.
            С нами вы научитесь говорить на английском легко и уверенно — даже если раньше это казалось невозможным!`,
    },
    {
        id: 3,
        title: 'Индивидуальное. Дети',
        imagePaths: [
            img35,
            Individual_teens1,
            Individual_teens2,
            Individual_kids1,
        ],
        description: `Персональные занятия для детей — наилучший способ раскрыть потенциал ребёнка в изучении английского. Индивидуальный подход помогает адаптировать программу под интересы и особенности каждого ученика.
Индивидуальные занятия для детей:
Индивидуальный подход к каждому ребенку.
Гибкий график занятий: 2 или 3 раза в неделю.
Возможные форматы:
Только с носителем языка.
Комбинация занятий с носителем и российским филологом.
Только с российским преподавателем.
Занятия проходят в увлекательной игровой форме, что помогает детям быстрее усваивать материал.
Мы поможем вашему ребёнку полюбить английский и добиться отличных результатов уже в первый учебный год.`,
    },
    {
        id: 4,
        title: 'Индивидуальное обучение (от 16 лет)',
        imagePaths: [
            Individual_adults1,
            Individual_adults2,
            Individual_adults3,
            Individual_adults4,
            img37,
        ],
        description: `Хотите освоить английский максимально эффективно? Индивидуальные занятия — идеальный вариант для тех, кто ценит своё время и хочет видеть быстрый результат.
Персональные занятия английским для подростков и взрослых:
Полностью индивидуальный график.
Гибкие программы: выбирайте преподавателей — носитель языка, российский филолог или комбинированный вариант.
Интенсивность: 2 или 3 раза в неделю.
Максимальный результат в короткие сроки — занятия полностью адаптированы под ваши цели: работа, учёба, путешествия, экзамены.
Инвестируйте в своё развитие — знания английского откроют перед вами новые профессиональные и личные возможности.`,
    },
    {
        id: 5,
        title: 'Бизнес Английский',
        imagePaths: [
            img22,
            Business_English1,
            Business_English2,
            Business_English3,
            Business_English4,
            img36,
        ],
        description: `Деловой английский — это не просто язык, это инструмент для развития вашей карьеры, успешных переговоров и уверенного общения с международными партнёрами.
Деловой английский для профессионалов:
Мини-группы 1+1 или индивидуально.
Интенсивность: 2 или 3 занятия в неделю.
Основной акцент на практическую деловую лексику, навыки ведения переговоров, деловой переписки, презентаций.
Форматы занятий:
Только с носителем.
Комбинация носителей и российских филологов.
Расширяйте свои горизонты — свободное владение деловым английским поможет вам достичь новых высот в профессии.`,
    },
    {
        id: 6,
        title: 'Английский для программистов',
        imagePaths: [
            English_programmers1,
            English_programmers2,
            English_programmers3,
        ],
        description: `Для специалистов IT-сферы владение английским — неотъемлемая часть профессионального роста. Мы предлагаем специализированный курс для разработчиков любого уровня.
Английский для IT-специалистов:
Специализированная лексика, термины, интервью на английском, работа с документацией.
Форматы занятий — как в бизнес-английском.
Комбинированный подход: носители языка и российские филологи.
Уверенное владение английским откроет перед вами доступ к лучшим вакансиям и проектам по всему миру.`,
    },
    {
        id: 7,
        title: 'Медицинский английский',
        imagePaths: [img25, Medical_English1, Medical_English2],
        description: `Откройте для себя медицинский английский — ключевой навык для успешной международной карьеры в сфере здравоохранения.
Профессиональный английский для врачей и медицинских специалистов:
Интенсивная программа 2–3 раза в неделю.
Занятия с носителями языка и российскими преподавателями.
Изучаем профильную медицинскую лексику, общение с пациентами, подготовку к международной сертификации.
Получите уверенность в профессиональном английском и будьте готовы к работе в любой точке мира.`,
    },
    {
        id: 8,
        title: 'Подготовка к ОГЭ',
        imagePaths: [oge_1, oge_2, oge_3, oge_4, oge_5],
        description: `Хотите, чтобы ваш ребенок сдал ОГЭ на высокие баллы? Мы знаем, как помочь добиться максимального результата с уверенностью.
Качественная подготовка к ОГЭ по английскому языку:
2 или 3 занятия в неделю.
Комбинация носителей языка и российских преподавателей.
Практика всех частей экзамена: аудирование, чтение, письмо, говорение.
Постоянный контроль прогресса.
Уверенные знания, грамотная речь и высокий балл — вместе мы добьёмся этого результата!`,
    },
    {
        id: 9,
        title: 'Подготовка к ЕГЭ',
        imagePaths: [ege_1, ege_2, ege_3, ege_4],
        description: `Подготовка к ЕГЭ — это не только изучение языка, но и системный подход к сдаче экзамена. Мы подберём оптимальную программу для каждого ученика.
Подготовка к ЕГЭ по английскому языку:
Занятия 2–3 раза в неделю.
Форматы — как при подготовке к ОГЭ.
Подробный разбор всех типов заданий, практика устной и письменной речи.
Ликвидация пробелов в грамматике и лексике.
С нами вы подготовитесь к ЕГЭ на высокие баллы и поступите в желаемый вуз.`,
    },
    {
        id: 10,
        title: 'Speaking Club',
        imagePaths: [img29, img30, Speaking_club1],
        description: `Хотите свободно общаться на английском? Наш Speaking Club — это регулярная практика с носителями языка в дружеской атмосфере.
Разговорный клуб с носителями языка:
Занятия в мини-группах до 5 человек.
2 раза в неделю.
Полное погружение в разговорную практику.
Развитие беглой речи, снятие языкового барьера, улучшение произношения.
Приходите на Speaking Club — начните говорить свободно уже с первого месяца занятий!`,
    },
];
